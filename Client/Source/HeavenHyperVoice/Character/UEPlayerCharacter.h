...
class HEAVENHYPERVOICE_API AUEPlayerCharacter : public APawn
{
  ...
  public:
	...
	bool IsRemoteProxy() const
	{
		return bIsRemoteProxy;
	}
	...
	bool IsAppearanceExternallyDriven() const
	{
		return bAppearanceExternallyDriven;
	}

	// 스냅샷이 알려준 목표 지점. Tick 이 보간으로 따라간다. 20Hz 라 그대로 박으면
	// 끊겨 보인다.
	void ApplyRemoteCoreState(const hhv::movement::State &State, double ServerTime, bool bTeleported);
	...
	UFUNCTION(BlueprintPure, Category = "Character|State")
	FGameplayTag GetCharacterStateTag() const
	{
	...
	UUEFieldWildPokemonSyncComponent *GetFieldWildPokemonSyncComponent() const
	{
		return FieldWildPokemonSyncComponent;
	}

	UFUNCTION(BlueprintPure, Category = "Field Server")
	UUEFieldRemotePlayerSyncComponent *GetFieldRemotePlayerSyncComponent() const
	{
		return FieldRemotePlayerSyncComponent;
	}

	UUEFieldPartnerSyncComponent *GetFieldPartnerSyncComponent() const
	{
		return FieldPartnerSyncComponent;
	}

	UFUNCTION(BlueprintPure, Category = "Animation")
	UUEPlayerAnimationDataAsset *GetPlayerAnimationData() const
	{
		return PlayerAnimationData;
	}

	UFUNCTION(BlueprintPure, Category = "Customization")
	EUEHHVGender GetCustomizationGender() const
	{
		return CurrentCustomizationGender;
	}
  ...
  private:
	...
	void ApplyHHVColorToSlots(USkeletalMeshComponent *Component, const FLinearColor &Color,
	                          const TArray<FString> &SlotContains) const;
	void ApplyHHVEyeMaterial(USkeletalMeshComponent *Component, const FUEHHVCustomizationOption &EyeOption,
	...
	UPROPERTY(Transient, BlueprintReadOnly, Category = "Character|State",
	          meta = (AllowPrivateAccess = "true"))
