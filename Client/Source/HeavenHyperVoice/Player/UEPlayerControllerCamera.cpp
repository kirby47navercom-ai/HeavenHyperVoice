...
void AUEPlayerController::BindPhotoInput(UEnhancedInputComponent *Input)
{
	...
	{
		Input->BindAction(PhotoCaptureAction, ETriggerEvent::Started, this, &ThisClass::TakePhoto);
	}
	if (PhotoZoomAction)
	{
		Input->BindAction(PhotoZoomAction, ETriggerEvent::Triggered, this, &ThisClass::HandlePhotoZoom);
	}
	if (PhotoLookAction)
	{
		Input->BindAction(PhotoLookAction, ETriggerEvent::Started, this, &ThisClass::HandlePhotoLookStarted);
		Input->BindAction(PhotoLookAction, ETriggerEvent::Completed, this,
		                  &ThisClass::HandlePhotoLookStopped);
...
void AUEPlayerController::EnterPhotoMode()
{
	...
	if (bPhotoMode || bReturningToFrontend || !IsLocalController() || !PlayerCharacter ||
	    !PhotoModeWidgetClass || !PhotoMappingContext)
	{
		return;
	}
	if (bInCombat || PlayerCharacter->IsRolling() ||
	    !PlayerCharacter->GetCoreMovement()->IsMovingOnGround() ||
...
void AUEPlayerController::ExitPhotoMode()
{
	...
	if (PhotoProcessedHandle.IsValid())
	{
		...
		if (!PendingPhotoFilename.IsEmpty())
		{
	...
	for (auto &Entry : PhotoHiddenWorldWidgets)
	{
		if (Entry.Key.IsValid())
		{
			Entry.Key->SetRenderOpacity(Entry.Value);
		}
	}
	...
	if (AHUD *HUD = GetHUD())
	{
		HUD->bShowHUD = bPhotoSavedHUD;
	}
	if (PhotoCamera.IsValid())
	{
		PhotoCamera->SetFieldOfView(PhotoOriginalFOV);
	}
	PhotoCamera.Reset();
	PhotoPawn.Reset();
	PendingMovementInput = FVector2D::ZeroVector;
	PushMovementInputToCharacter();
	if (auto *PlayerCharacter = GetControlledPlayerCharacter())
	{
		PlayerCharacter->GetCoreMovement()->StopMovementImmediately();
	}
	RefreshWindowInput();
...
void AUEPlayerController::HideGameplayUIForPhoto()
{
	...
	{
		if (Widget == PhotoWidget || (Widget->GetOwningPlayer() && Widget->GetOwningPlayer() != this))
		{
			continue;
		}
		if (!PhotoHiddenWidgets.Contains(Widget))
		{
			PhotoHiddenWidgets.Add(Widget, Widget->GetVisibility());
		}
	...
	for (TObjectIterator<UWidgetComponent> It; It; ++It)
	{
		...
		{
			if (!PhotoHiddenWorldWidgets.Contains(Widget))
			{
				PhotoHiddenWorldWidgets.Add(Widget, Widget->GetRenderOpacity());
			}
                                                 ...
                                                 TSet<FPrimitiveComponentId> &HiddenComponents)
{
	...
	{
		HiddenComponents.Add(Component->GetPrimitiveSceneId());
	}
}
...
void AUEPlayerController::TickPhotoMode(float DeltaSeconds)
{
	...
	if (!PlayerCharacter || PhotoPawn.Get() != PlayerCharacter || !PhotoCamera.IsValid() || bInCombat ||
	    IsNearPhotoBoundary() ||
	    PlayerCharacter->GetCharacterStateTag() == UEGameplayTags::State_Character_Damage ||
	    PlayerCharacter->GetCharacterStateTag() == UEGameplayTags::State_Character_Death)
	{
...
void AUEPlayerController::RefreshPhotoInput()
{
	...
	else
	{
		...
		if (PhotoWidget)
		{
			Mode.SetWidgetToFocus(PhotoWidget->TakeWidget());
		}
		SetInputMode(Mode);
	}
}
...
void AUEPlayerController::HandlePhotoZoom(const FInputActionValue &Value)
{
	if (bPhotoMode)
	{
		SetPhotoZoom(PhotoZoom + Value.Get<float>() * .08f);
	}
}
...
void AUEPlayerController::SetPhotoZoom(float Value)
{
	...
	const float Target = FMath::Lerp(PhotoWideFOV, FMath::Min(PhotoTeleFOV, PhotoWideFOV), PhotoZoom);
	const float Magnification = FMath::Tan(FMath::DegreesToRadians(PhotoWideFOV * .5f)) /
	                            FMath::Tan(FMath::DegreesToRadians(Target * .5f));
	if (PhotoWidget)
	{
		PhotoWidget->UpdateZoom(PhotoZoom, Magnification);
	}
}

void AUEPlayerController::TakePhoto()
{
	if (!bPhotoMode || !PendingPhotoFilename.IsEmpty() || FScreenshotRequest::IsScreenshotRequested())
	{
		return;
	}
	FString Folder;
	...
	if (Folder.IsEmpty())
	{
		Folder = FPaths::ConvertRelativePathToFull(FPaths::ScreenShotDir());
	}
	...
	if (!IFileManager::Get().MakeDirectory(*Folder, true))
	{
		if (PhotoWidget)
		{
			PhotoWidget->SetStatus(FText::FromString(TEXT("사진 폴더를 만들지 못했습니다.")));
		}
		return;
	}
	PendingPhotoFilename =
	    FPaths::Combine(Folder, TEXT("HHV_") + FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S_")) +
	                                FGuid::NewGuid().ToString(EGuidFormats::Digits).Left(8) + TEXT(".png"));
	PhotoProcessedHandle =
	    FScreenshotRequest::OnScreenshotRequestProcessed().AddUObject(this, &ThisClass::OnPhotoProcessed);
	if (PhotoWidget)
	{
		PhotoWidget->SetStatus(FText::FromString(TEXT("사진 저장 중…")));
	}
...
void AUEPlayerController::OnPhotoProcessed()
{
	...
	if (PhotoWidget)
	{
		PhotoWidget->SetStatus(FText::FromString(bSaved ? TEXT("사진 저장됨 · ") + PendingPhotoFilename
		                                                : TEXT("사진을 저장하지 못했습니다.")));
	}
	UE_LOG(LogTemp, Display, TEXT("Photo %s: %s"), bSaved ? TEXT("saved") : TEXT("failed"),
	       *PendingPhotoFilename);
	PendingPhotoFilename.Empty();
}
