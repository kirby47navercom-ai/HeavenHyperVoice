...
public:
    ...
    //
    // preferredRoomId 가 0 이 아니면 그 방을 먼저 시도한다. 파티가 같은 방으로
    // 들어가는 경로다. 그 방이 없거나 꽉 찼으면 평소대로 배정한다 — 파티를
    // 못 모았다고 입장 자체를 막지는 않는다.
    //
    // Redis 를 여기서 읽지 않는다. 이 클래스는 I/O 없는 순수 로직으로 남긴다.
    Room* join(std::uint32_t type, std::uint32_t preferredRoomId = 0);
