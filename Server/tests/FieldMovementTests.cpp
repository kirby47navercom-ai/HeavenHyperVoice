...
struct Geometry {
    std::filesystem::path path = std::filesystem::temp_directory_path() /
        ("hhv-nav-test-" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()) + ".hhvmap");
    std::ofstream out{path};
    Geometry() { out << "settings 25 2 34 88 45 44\n"; }
    ~Geometry() { out.close(); std::error_code error; std::filesystem::remove(path, error); }

    void rectangle(float x0, float y0, float x1, float y1, float base = 0.f, float slope = 0.f) {
        const auto point = [&](float x, float y) { out << x << ' ' << y << ' ' << base + slope * x << ' '; };
        out << "triangle "; point(x0, y0); point(x1, y0); point(x1, y1); out << "ground\n";
        out << "triangle "; point(x0, y0); point(x1, y1); point(x0, y1); out << "ground\n";
    }

    void load(heaven::Map& map) {
        out.close();
        std::string error;
        if (!map.loadFromFile(path.string(), error)) throw std::runtime_error(error);
    }
};
...
void slopeAndLayers() {
    ...
    heaven::Map map;
    geometry.load(map);
    auto from = ground(map, 200, 500, -11800);
    for (int x = 205; x <= 1700; x += 5) {
        ...
        if (map.blockedAlong(from, to, map.agent())) {
            throw std::runtime_error("continuous slope blocked x=" + std::to_string(from.x) + " -> " +
                std::to_string(to.x) + " z=" + std::to_string(from.z) + " -> " + std::to_string(to.z));
        }
        from = to;
    }
    ...
    require(!map.canStandAt(-10, 500, map.agent()), "standability silently snapped outside XY");
    require(map.nearestStandable(-10, 500, 200, map.agent(), near, -12000), "explicit nearest fallback failed");
...
void cornersAndParallelPaths() {
    ...
    for (int tick = 0; tick < 1000; ++tick) {
        ...
        if (partner.teleportedThisTick) {
            std::cerr << "Route:";
            for (const auto& point : path.points) std::cerr << " (" << point.x << ',' << point.y << ')';
            std::cerr << '\n';
        ...
        require(!map.blockedAlong(previous, partner.location, map.agent()), "partner cut a corner");
    }
    require(std::hypot(partner.location.x - goal.x, partner.location.y - goal.y) <= 46,
            "partner failed to finish its route");
    std::cout << "PASS 800 concurrent paths and partner corner following\n";
}
...
void exportedMap(const std::filesystem::path& file, float origin) {
    heaven::Map map;
    std::string error;
    if (!map.loadFromFile(file.string(), error)) throw std::runtime_error(error);
    const auto start = ground(map, origin, origin);
    using namespace heaven::fieldshared;
    PartnerOwnerState owner{start, {}, 0};
    PartnerState partner;
    require(PartnerFollower::initialize(owner, 20, partner, &map), "exported map partner initialization failed");
    ...
    std::cout << "PASS " << file.filename().string() << " 10m player path and partner follow; polys="
...
int main(int argc, char** argv) {
    try {
        ...
        if (argc > 1) {
            ...
            exportedMap(std::filesystem::path(argv[1]).parent_path() / "PlayerTestLevel.hhvmap", 25600.f);
        }
        return 0;
