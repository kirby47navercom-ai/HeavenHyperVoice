...
로그인·채팅·필드 서버. 전부 IOCP + TLS 위에서 돌고, 클라이언트는 로그인해서
**캐릭터를 고른 뒤** 받은 서명 티켓으로만 들어갈 수 있다.

```
가입(ID/PW) → 로그인 → 로비에서 캐릭터 선택 또는 생성 → 티켓 → 필드 입장
```
...
| `Data/` | 계정·캐릭터 저장소 (ODBC). LoginServer 와 FieldServer 가 함께 쓴다 |
| `LoginServer/` | 자격증명 확인 → 캐릭터 선택 → 티켓 발급. 기본 포트 **9100** |
| `ChatServer/` | 티켓 검증 → 채팅방. 기본 포트 **9000** |
| `FieldServer/` | 티켓 검증 → 이동 중계. 섹터 격자 + 시야 목록. 기본 포트 **9200** |
...
| `audience` | 이 티켓이 통용되는 서비스(`field`/`chat`/추후 `voice`). **없으면 필드 티켓을 채팅 서버에 재사용할 수 있다** |
...
지금은 클라이언트가 필드와 채팅에 각각 직접 붙는다. 인스턴스 서버까지 늘어나면
...
`pos:` 는 필드 서버가 쓴다. 위 *필드* 절 참고. `inst:` 는 인스턴스 서버가 생길 때다.
...
```powershell
.\build\windows-x64\bin\Debug\LoginServer.exe --port 9100 --chat-port 9000 --field-port 9200
