...
| `Movement/` | 공통 이동 코어 어댑터와 Recast/Detour NavMesh 길찾기 |
...
`argon2`, `hiredis`, `lua`, `sol2`, `recastnavigation`. ODBC 드라이버 매니저와 자격증명 관리자는 Windows SDK 구성요소라
...
상위 상태는 **C++ FSM**, 상태 전환 판단과 행동 선택은 **상태별 Lua BT**가 맡는다.
이전 FSM 커밋 `4fdfc392`의 역할 분리를 참고하되, 전환 조건은 C++에 두지 않는다.
현재 상태는 `Wander`와 `Battle`이다. 매 서버 틱에 현재 상태의 BT 하나를 평가하며,
`continue`를 반환하면 기존 배회 방향 또는 전투 추격 경로를 이어 간다.

배회는 임시로 **A* 없이 랜덤 방향으로 1~3초 이동하고 1~3초 대기**하는 방식을 쓴다.
이동·대기 시간 범위는 `wander.lua`에서 정하고, C++ 실행기가 매 이동 시작에 방향을 뽑는다.
벽에 막히거나 활동 영역 경계에 도달하면 일찍 이동을 끝내고 뽑아 둔 시간만큼 대기한다.
배회에서는 목적지 지면 조회·경로 탐색·우회 경로 검사를 하지 않으며, 실제 충돌과 접지는
월드의 공통 이동 코어가 처리한다. 이동·대기 중에도 Lua의 플레이어 감지는 계속된다.

| 파일 | 역할 |
|---|---|
| `src/WildPokemonAIFSM.cpp` | 상태 보관과 요청된 전환 적용만 담당 |
| `src/WildPokemonAIWanderAction.cpp` | 랜덤 방향으로 시간제 배회, 경계 도달 시 대기 |
| `src/WildPokemonAIBattleAction.cpp` | 추격·공격 명령 실행과 실행 대기 시간 관리 |
| `src/WildPokemonAIAction.cpp` | 추격 경로 실행, 이동 상태와 행동 타이머 |
| `src/WildAi.cpp` | 개체별 FSM·행동 메모리와 Lua/실행기 연결 |
| `src/WildBt.cpp` | 서버 관측 정보를 Lua에 전달하고 반환 명령을 읽음 |
| `scripts/wild_ai.lua` | 상태별 Lua 모듈을 등록하는 진입점 |
| `scripts/wild_pokemon_ai/wander.lua` | 배회 BT, 플레이어 감지와 Battle 전환 판단 |
| `scripts/wild_pokemon_ai/battle.lua` | 전투 BT, 추격·공격과 Wander 복귀 판단 |
| `scripts/wild_pokemon_ai/perception.lua` | 플레이어 좌표로 거리 계산과 대상 선택 |

Lua에는 현재 개체의 좌표·종족·맵 식별값, 같은 방/맵 플레이어의 ID와 좌표 목록,
현재 대상 ID, 이동 여부, 경로 목표, 휴식·재탐색·공격 대기 시간이 전달된다.
거리 계산은 기존과 같은 XY 평면 기준이며 Lua에서 수행한다. Z도 컨텍스트에 들어 있다.
현재 감지 거리 900cm, 추적 해제 거리 1800cm, 공격 거리 180cm는 상태별 Lua의 상수다.
서버는 대상을 대신 고르거나 감지 거리로 상태를 바꾸지 않는다.

Lua 반환값의 `next_state`는 `wander` 또는 `battle`이며, 생략하면 현재 상태를 유지한다.
`action`은 `continue`, `wait`, `wander`, `chase`, `attack` 중 하나다.
전환 요청이 있으면 기존 경로·대기 시간을 정리하고 새 대상 ID만 전달한다.
전환된 상태의 BT는 다음 서버 틱에 실행해 한 틱 안에서 상태가 연쇄 전환되지 않게 한다.
`wait`는 새 휴식을 시작하는 명령이 아니라 현재 이동을 멈추고 기다리는 명령이다.

FSM에는 플레이어 감지나 거리 조건이 없다. C++ 행동 실행기는 유효한 대상과 매개변수를
확인하고, 월드는 공격 거리와 대상의 소속을 최종 검증한다. 실제 속도·경사·충돌은
기존 공통 이동 코어가 처리한다. 새로운 상태를 추가할 때는 FSM 상태값, C++ 행동 실행기,
Lua 상태 BT 및 연결부 등록을 함께 추가한다.

추격과 파트너 따라오기는 **Recast/Detour NavMesh**를 사용한다. 맵 로드 때
`ServerGround` / `ServerWall` 형상이 담긴 기존 `.hhvcollision` 파일에서 NavMesh를 만들고,
이후 연결된 폴리곤을 조회한다. 기존 자체 격자 A*와 간선마다 공통 코어로 미리 걷게 하던
검사는 제거했다. 실제 캐릭터 이동과 높이는 계속 공통 코어가 계산한다.
경로 조회는 동기식이며 검색 한도, 경로 재사용, 재탐색 대기 시간을 적용한다.
맵 객체의 NavMesh는 방들에서 공유하고 각 조회의 검색 상태는 분리한다.

NavMesh 생성은 서버 준비 전에 한 번 실행되며 큰 맵에서는 시작 시간이 길어질 수 있다.
현재 디스크 캐시는 사용하지 않는다. 저장 방법과 생성 설정은
[공통 충돌 및 NavMesh 문서](../Client/Source/MovementCore/MAP_EXPORT.md)를 참고한다.
입력 공백 예측과 적체 강제 동기화는 아래 공통 이동 코어 문서에 설명한다.

`WildPokemonAITests`는 Lua 전환 정책, 공격 대기, 오류 응답, 개체별 상태 격리 및
실제 Filed 충돌 데이터에서 공통 코어 배회 이동을 검사한다.
배회 시간 만료·대기·재시작·벽 막힘·활동 경계도 검사하며, 추격 회귀 검사는 고정 출발점을 쓴다.
600cm 높이 차이가 있는 경사에서 목표 층을 선택하고 공통 코어로 끝까지 추격하는지도 검사한다.
`FieldMovementTests`는 실제 Filed/Goldenrod 이동, 타일 연결, 벽 우회, 단차, 겹친 층,
동시 경로 조회 및 파트너 모서리 이동을 검사한다.
이전 실패 재현 위치인 Filed의 배회 종료점 `(-159.323, 352.110, -12547.6)`에서
`(160.677, 352.110, -12578.4)`로 추격할 때, 테스트 속도 100cm/s로 6초 동안 이동한 뒤에도
공격 거리까지 도달하지 못한 사례가 있었다. NavMesh 교체 후 같은 위치를 별도로 검사해
공통 코어 경로 추종이 6초 이내 공격 거리 180cm에 들어가는 것을 확인했다.
...
클라이언트는 60Hz 입력과 예측 좌표를 보낸다. 서버는 공통 코어로 입력을 재실행하고 좌표를 비교한다.
입력이 비면 최대 100ms 동안 마지막 방향을 예측하고 이후에는 중립 입력으로 감속한다.
입력 확인에는 추측한 미래 상태가 아닌 실제 입력을 처리한 상태를 보낸다.
큐가 24개를 넘으면 미처리 입력을 모두 폐기하고 마지막 서버 확정 상태로 강제 보정한다.
서버가 입력 세대를 바꾸면 클라이언트도 예측 기록·미처리 버튼·누적 시간을 비우고 그 상태를 그대로 적용한다.
이전 세대의 지연 패킷과 보정은 무시하고, 새 세대의 입력 순번 1부터 재개한다.
클라이언트도 미확인 입력이 120개 쌓이면 같은 강제 동기화를 요청한다.
일반 보정에서는 확인된 순번 이후 입력만 재실행한다. 양쪽 실행 파일의 코어 버전은 3으로 맞춰야 한다.
필드와 인스턴스 모두 실제 서버 경과 시간으로 처리 예산을 채우며 개체당 한 번에 최대 6스텝을
실행한다. 자세한 제한과 테스트는 [공통 이동 코어](../Client/Source/MovementCore/README.md)를 참고한다.
...
`--wild-ai-script` 에 절대 경로를 주면 된다. 상태별 Lua 모듈은 해당 진입 파일의
디렉터리를 기준으로 찾는다. 배포할 때 `wild_ai.lua`와 `wild_pokemon_ai` 폴더를 함께 복사한다.
