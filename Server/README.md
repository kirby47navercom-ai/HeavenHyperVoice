...
배회 A* 연산은 제거했지만 전투 추격의 동기 경로 탐색은 유지된다.
입력 공백 예측과 적체 압축은 아래 공통 이동 코어 문서에 설명한다.
...
추가 조사 대상: Filed의 배회 종료점 `(-159.323, 352.110, -12547.6)`에서
`(160.677, 352.110, -12578.4)`로 추격할 때, 테스트 속도 100cm/s로 6초 동안 이동한 뒤에도
공격 거리까지 도달하지 못한 사례가 있었다. 이번 배회 변경은 해당 추격 지형 문제를 해결하지 않는다.
...
큐가 24개를 넘으면 전환 입력을 보존하면서 오래된 연속 이동을 생략한다. 보정에는 생략한 입력의
누적 수도 명시하며, 클라이언트는 해당 순번까지 제거한 뒤 나머지를 재실행한다.
